...
# Se incorpora el logo para ambas opciones (asegúrate de que el archivo "logo.png" esté en la carpeta)
...
<body>
  ...
  <div class="seccion info-req">
    <h2>Información sobre las ratios</h2>
    <ul>
      <li><b>Atención Directa</b>: Mínimo 0,47 (EJC) por residente.</li>
      <li><b>Gerocultores</b>: Mínimo 0,33 (EJC) por residente.</li>
      <li><b>Fisioterapeuta y Terapeuta Ocupacional</b>:
          4h/día (20h/sem) hasta 50 plazas. Por cada 25 plazas adicionales, +2h/día (10h/sem).
      </li>
      <li><b>Trabajador Social</b>: Contratación obligatoria (> 0h).</li>
      <li><b>Médico</b>: Presencia L-V (≥5h/sem).</li>
      <li><b>Enfermería</b>: 24h/día, 7d/sem (≥168h/sem).</li>
      <li><b>Atención No Directa</b>: Mínimo 0,15 (EJC) por residente.</li>
      <li><b>Psicólogo/a y Animador sociocultural</b>: Servicios opcionales.</li>
    </ul>
  </div>
...
# ===============================================================
    ...
    if st.button("📌 Calcular Ratio (Orden 2680-2024)"):
        ...
        coste_por_persona = 17000 * 1.32  # 22440 €/año
    ...
    if st.session_state.get("orden2680_calculated"):
        ...
        cumple_2680 = (rd2 >= rmin2)

        st.subheader("📊 Resultado del Cálculo de Ratio (Atención Directa)")
        st.write(f"**Total de EJC de Atención Directa:** {td2:.2f}")
        st.write(f"**Ratio de Atención Directa** (EJC por residente): {rd2:.2f}")
        st.write(f"**Ratio Mínima Requerida:** {rmin2:.2f}")
        st.write(f"**EJC requeridos:** {r2['ejc_requerido']:.2f}")
        st.write(f"**Déficit de EJC:** {r2['deficit']:.2f}")
        st.write(f"**Coste adicional anual estimado:** {r2['coste_adicional']:.2f} € (Coste por persona: {r2['coste_por_persona']:.2f} €/año)")

        st.markdown(f"""
        <p style='font-size:18px; color:{"green" if cumple_2680 else "red"};'>
          - El centro {"<b>CUMPLE ✅</b>" if cumple_2680 else "<b>NO CUMPLE ❌</b>"}
        </p>
        """, unsafe_allow_html=True)

        st.info(
            "Esta verificación se basa únicamente en la ratio global de personal de atención directa. "
            "Pueden existir otros requisitos (turnos, distribución de categorías...) "
            "contemplados en la Orden 2680-2024."
        )

        # Opción de guardar en HTML
        st.markdown("---")
        ...
        if guardar_orden:
            ...
            if st.button("Generar HTML (Orden 2680-2024)"):
                ...
                str_cumple_2680 = "Sí" if cumple_2680 else "No"
...
<body>
  ...
  <h2>Resultado</h2>
  <p><b>Total EJC de Atención Directa:</b> {td2:.2f}</p>
  <p><b>Ratio de Atención Directa (EJC/residente):</b> {rd2:.2f}</p>
  <p><b>Ratio Mínima Requerida:</b> {rmin2:.2f}</p>
  <p><b>EJC requeridos:</b> {r2['ejc_requerido']:.2f}</p>
  <p><b>Déficit de EJC:</b> {r2['deficit']:.2f}</p>
  <p><b>Coste adicional anual estimado:</b> {r2['coste_adicional']:.2f} €</p>
  <p>¿Cumple? 
     <span class='{"verde" if cumple_2680 else "rojo"}'>{str_cumple_2680}</span>
  </p>
