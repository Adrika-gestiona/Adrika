...
def formatear_numero(valor):
    """
    Formatea un número con dos decimales, usando punto como separador de miles
    y coma como separador decimal. Ejemplo: 644706.77 -> "644.706,77"
    """
...
if modo_calculo == "Cálculo RATIO CAM AM atención residencial":
    ...
    if st.session_state.get("cam_calculated"):
        ...
        st.markdown(
            f"<p style='color:{color_directa}; font-weight:bold;'>"
            f"Atención Directa (mínimo 0,47): {formatear_numero(rd/100)} → {si_cumple_texto(cumple_directa)}"
            f"</p>",
        ...
        st.markdown(
            f"<p style='color:{color_nodirecta}; font-weight:bold;'>"
            f"Atención No Directa (mínimo 0,15): {formatear_numero(rnd/100)} → {si_cumple_texto(cumple_nodirecta)}"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<p style='color:{color_gero}; font-weight:bold;'>"
            f"Gerocultores (mínimo 0,33): {formatear_numero(ratio_gero)} → {si_cumple_texto(cumple_gero)}"
            f"</p>",
            unsafe_allow_html=True
        )

        st.subheader("🩺 Cálculo de horas Fisioterapia y Terapia Ocupacional")
        st.write(f"**Plazas ocupadas:** {formatear_numero(res['ocupacion'])} residentes")
        ...
        st.markdown(
            f"<p style='color:{color_fisio}; font-weight:bold;'>"
            f"Fisioterapeuta → Horas requeridas/semana: {formatear_numero(horas_req_terapia)} | "
            f"Horas introducidas: {formatear_numero(h_fisio)} → {si_cumple_texto(cumple_fisio)}"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<p style='color:{color_to}; font-weight:bold;'>"
            f"Terapeuta Ocupacional → Horas requeridas/semana: {formatear_numero(horas_req_terapia)} | "
            f"Horas introducidas: {formatear_numero(h_to)} → {si_cumple_texto(cumple_to)}"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<p style='color:{color_ts}; font-weight:bold;'>"
            f"Trabajador Social: {formatear_numero(horas_ts)} h/sem → {si_cumple_texto(cumple_ts)} (mínimo > 0)"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<p style='color:{color_med}; font-weight:bold;'>"
            f"Médico: {formatear_numero(horas_med)} h/sem → {si_cumple_texto(cumple_med)} (mínimo 5h/sem)"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<p style='color:{color_enf}; font-weight:bold;'>"
            f"Enfermería (ATS/DUE): {formatear_numero(horas_enf)} h/sem → {si_cumple_texto(cumple_enf)} (mínimo 168h/sem)"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        if guardar_cam:
            ...
            # Usamos formatear_numero(...) en la exportación HTML también
...
<body>
  ...
  <p><b>Plazas ocupadas:</b> {formatear_numero(res['ocupacion'])} residentes</p>
  <div>
    <h2>Horas Introducidas</h2>
    <table>
      <tr><th>Categoría</th><th>Horas/sem</th></tr>
      {''.join(f"<tr><td>{cat}</td><td>{formatear_numero(h)} h/sem</td></tr>" for cat, h in res["horas_directas"].items())}
    </table>
    <table>
      <tr><th>Categoría</th><th>Horas/sem</th></tr>
      {''.join(f"<tr><td>{cat}</td><td>{formatear_numero(h)} h/sem</td></tr>" for cat, h in res["horas_no_directas"].items())}
    </table>
  </div>
  <div>
    <h2>Resultados Principales</h2>
    <p>Atención Directa → Total EQ: <b>{formatear_numero(td)}</b> | Ratio: <b>{formatear_numero(rd)}</b> por cada 100 residentes</p>
    <p>Atención No Directa → Total EQ: <b>{formatear_numero(tnd)}</b> | Ratio: <b>{formatear_numero(rnd)}</b> por cada 100 residentes</p>
    <p>Gerocultores → Ratio: <b>{formatear_numero(ratio_gero)}</b> (Mínimo 0,33) - {"Sí (>=0,33)" if cumple_gero else "No (<0,33)"} </p>
  </div>
  <div>
    <h2>Verificación de cumplimiento</h2>
    <p>Atención Directa: {"Sí (>=0,47)" if (rd/100) >= 0.47 else "No (<0,47)"} (Mínimo 0,47). Ratio: <b>{formatear_numero(rd/100)}</b></p>
    <p>Atención No Directa: {"Sí (>=0,15)" if (rnd/100) >= 0.15 else "No (<0,15)"} (Mínimo 0,15). Ratio: <b>{formatear_numero(rnd/100)}</b></p>
  </div>
  <hr>
...
# ===============================================================
    ...
    if st.session_state.get("orden2680_calculated"):
        ...
        st.markdown(
            f"<p style='color:{color_orden}; font-weight:bold;'>"
            f"{si_cumple_texto(cumple_orden)} (mínimo {formatear_numero(rmin2)}) | "
            f"Ratio: {formatear_numero(rd2)}"
            f"</p>",
            unsafe_allow_html=True
        )
        ...
        if r2["deficit"] > 0:
            ...
            st.markdown(
                "<p style='font-size:18px; color:green;'>El centro CUMPLE con la ratio mínima requerida.</p>",
                unsafe_allow_html=True
            )
...
<body>
  ...
  <p><b>Plazas autorizadas/ocupadas:</b> {formatear_numero(r2['ocupacion'])}</p>
  <h2>Horas Introducidas (Atención Directa)</h2>
  <table>
    <tr><th>Categoría</th><th>Horas/sem</th></tr>
    {''.join(f"<tr><td>{cat}</td><td>{formatear_numero(h)} h/sem</td></tr>" for cat, h in r2["horas_directas"].items())}
  </table>
  <h2>Resultado</h2>
