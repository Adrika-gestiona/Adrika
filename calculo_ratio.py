...
# INTERFAZ PRINCIPAL
# ---------------------------------------------------------------------------------------------
# Cabecera: Enlaces a www.mayores.ai con formato de hipervínculo (azul y subrayado)
st.markdown(
    "<h4 style='text-align: center;'><a href='https://www.mayores.ai' target='_blank' style='color: blue; text-decoration: underline;'>www.mayores.ai</a> - Ádrika</h4>",
    ...
    if st.session_state.get("cam_calculated"):
        ...
        if guardar_cam:
            ...
            if st.button("Generar HTML (CAM AM)"):
                ...
                str_directa = "Sí (>=0,47)" if (rd/100) >= 0.47 else "No (<0,47)"
                str_nodirecta = "Sí (>=0,15)" if (rnd/100) >= 0.15 else "No (<0,15)"
...
<body>
  ...
  <div>
    ...
    <p>Gerocultores → {str_gero} (Mínimo 0,33). Ratio: <b>{ratio_gero:.2f}</b></p>
  </div>
  <div>
    <h2>Verificación de cumplimiento</h2>
    <p>Atención Directa: {str_directa} (Mínimo 0,47). Ratio: <b>{(rd/100):.2f}</b></p>
    <p>Atención No Directa: {str_nodirecta} (Mínimo 0,15). Ratio: <b>{(rnd/100):.2f}</b></p>
  </div>
  <hr>
  <p>Informe generado automáticamente desde la aplicación (CAM AM).</p>
  <p style="text-align: center;">mayores.ai - Ádrika</p>
</body>
</html>"""
                st.download_button(
...
<body>
  ...
  <p style="text-align: center;"><a href="https://www.mayores.ai" target="_blank" style="color: blue; text-decoration: underline;">mayores.ai - Ádrika</a></p>
</body>
</html>"""
                st.download_button(
                    label="Descargar HTML",
...
# ---------------------------------------------------------------------------------------------
# Al final de la aplicación, en la interfaz principal, se añade el texto de marca con enlace
st.markdown("<p style='text-align: center;'><a href='https://www.mayores.ai' target='_blank' style='color: blue; text-decoration: underline;'>mayores.ai - Ádrika</a></p>", unsafe_allow_html=True)