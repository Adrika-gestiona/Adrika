...
def si_cumple_texto(cumple: bool) -> str:
    """ Devuelve el texto con icono, sin color (para usar dentro de HTML, etc.). """
    return "✅ CUMPLE" if cumple else "❌ NO CUMPLE"

# >>> CAMBIO <<<
def texto_en_color(texto: str, cumple: bool) -> str:
    """
    Envuelve un texto en un <span> con color verde o rojo según si cumple o no.
    Esto se usará al mostrar resultados en Streamlit con `unsafe_allow_html=True`.
    """
    color = "green" if cumple else "red"
    return f"<span style='color:{color}; font-weight:bold;'>{texto}</span>"
...
if modo_calculo == "Cálculo RATIO CAM AM atención residencial":
    ...
    if st.session_state.get("cam_calculated"):
        ...
        # >>> CAMBIO <<< (usamos texto_en_color para poner el texto en verde/rojo)
        cumple_directa = (rd/100) >= 0.47
        color_directa = texto_en_color(si_cumple_texto(cumple_directa), cumple_directa)
        st.markdown(
            f"{color_directa} (Mínimo 0,47) - Atención Directa. Ratio: **{(rd/100):.2f}**",
            unsafe_allow_html=True
        )

        cumple_nodirecta = (rnd/100) >= 0.15
        color_nodirecta = texto_en_color(si_cumple_texto(cumple_nodirecta), cumple_nodirecta)
        st.markdown(
            f"{color_nodirecta} (Mínimo 0,15) - Atención No Directa. Ratio: **{(rnd/100):.2f}**",
            unsafe_allow_html=True
        )

        color_gero = texto_en_color(si_cumple_texto(cumple_gero), cumple_gero)
        st.markdown(
            f"{color_gero} (Mínimo 0,33) - Gerocultores. Ratio: **{ratio_gero:.2f}**",
            unsafe_allow_html=True
        )
        ...
        # >>> CAMBIO <<< Fisioterapeuta
        cumple_fisio = (h_fisio >= horas_req_terapia)
        color_fisio = texto_en_color(si_cumple_texto(cumple_fisio), cumple_fisio)
        st.markdown(
            f"🔹 Fisioterapeuta → Horas requeridas/semana: **{horas_req_terapia:.2f}** | "
            f"Horas introducidas: **{h_fisio:.2f}** → {color_fisio}",
            unsafe_allow_html=True
        )

        # >>> CAMBIO <<< Terapeuta Ocupacional
        cumple_to = (h_to >= horas_req_terapia)
        color_to = texto_en_color(si_cumple_texto(cumple_to), cumple_to)
        st.markdown(
            f"🔹 Terapeuta Ocupacional → Horas requeridas/semana: **{horas_req_terapia:.2f}** | "
            f"Horas introducidas: **{h_to:.2f}** → {color_to}",
            unsafe_allow_html=True
        )
        ...
        # >>> CAMBIO <<< Trabajador Social
        color_ts = texto_en_color(si_cumple_texto(cumple_ts), cumple_ts)
        st.markdown(
            f"Trabajador Social: Horas introducidas: **{horas_ts:.2f}** → {color_ts} (mínimo > 0)",
            unsafe_allow_html=True
        )

        # >>> CAMBIO <<< Médico
        color_med = texto_en_color(si_cumple_texto(cumple_med), cumple_med)
        st.markdown(
            f"Médico: Horas introducidas: **{horas_med:.2f}** → {color_med} (mínimo 5h/sem)",
            unsafe_allow_html=True
        )

        # >>> CAMBIO <<< Enfermería
        color_enf = texto_en_color(si_cumple_texto(cumple_enf), cumple_enf)
        st.markdown(
            f"Enfermería (ATS/DUE): Horas introducidas: **{horas_enf:.2f}** → {color_enf} (mínimo 168h/sem)",
            unsafe_allow_html=True
        )
...
# ===============================================================
    ...
    if st.session_state.get("orden2680_calculated"):
        ...
        # >>> CAMBIO <<< Coloreamos el texto
        color_orden = texto_en_color(si_cumple_texto(cumple_orden), cumple_orden)
        st.markdown(
            f"{color_orden} (Mínimo {formatear_numero(rmin2)}) | "
