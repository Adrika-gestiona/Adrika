...
<body>
  ...
  <div>
    ...
    <p>Gerocultores → {"Sí (>=0,33)" if (ratio_gero>=0.33) else "No (<0,33)"} (Mínimo 0,33). Ratio: <b>{formatear_numero(ratio_gero)}</b></p>
  </div>
  <div>
...
# ===============================================================
    ...
    if st.session_state.get("orden2680_calculated"):
        ...
        # Envolvemos todo en color, solo "✅ CUMPLE" / "❌ NO CUMPLE" va en negrita
        st.markdown(
        ...
        if r2["deficit"] > 0:
            ...
            # Mantengo el texto de explicación en rojo si hay déficit
