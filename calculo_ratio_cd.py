...
def calcular_horas_gerocultores(usuarios):
    """
    Calcula las horas mínimas requeridas para Gerocultores.
    Se asignan 225 horas por cada grupo completo de 35 usuarios,
    y para el resto se calcula la fracción (resto/35) * 225.
    
    Ejemplo: para 75 usuarios se calcula:
       bloques_completos = 75 // 35 = 2  
       resto = 75 % 35 = 5  
       horas_totales = 2*225 + (5/35)*225
    """
    bloques_completos = usuarios // 35
    resto = usuarios % 35
...
2. Para la categoría de **Gerocultor**, se exigirán **225 horas semanales** por cada **35 usuarios o fracción**.  
...
if st.button("📌 Calcular Ratio"):
    ...
    # Calcular las horas mínimas exigidas para Gerocultores según la fracción exacta
    ...
    # Guardamos los resultados en session_state para usarlos en el HTML
...
if st.session_state.get("centro_calculated"):
    ...
    <p style='font-size:18px; color:{"green" if res["cumple_gero"] else "red"};'>
        🔹 <b>Horas de Gerocultores</b>: {res["horas_gero"]:.2f} h/semana (mínimo requerido: {res["horas_min_gero"]:.2f} h/semana) → {si_cumple_texto(res["cumple_gero"])}
    </p>
    """, unsafe_allow_html=True)
    ...
    if guardar_informe:
        ...
        if st.button("Generar HTML Informe"):
            ...
            str_gero = f"Sí (>= {res['horas_min_gero']:.2f} h/sem)" if res["cumple_gero"] else f"No (< {res['horas_min_gero']:.2f} h/sem)"
...
<body>
  ...
  <div class="seccion">
    ...
    <p><b>Horas de Gerocultores:</b> {res["horas_gero"]:.2f} h/semana → <span class='{"verde" if res["cumple_gero"] else "rojo"}'>{str_gero}</span> (mínimo requerido: {res["horas_min_gero"]:.2f} h/semana)</p>
  </div>
