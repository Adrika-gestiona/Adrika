...
if view_mode == "Gastos":
    ...
    st.markdown("<h2 style='font-size:24px;'>Filtra por contrato, centro y año.</h2>", unsafe_allow_html=True)

    all_contratos = sorted(contracts_df["contrato"].unique().tolist())
    all_centros = sorted(contracts_df["centro"].unique().tolist())
    ...
    # Selección de contratos, centros y años
    ...
    if st.sidebar.checkbox("Seleccionar TODOS los centros", value=False):
        selected_centros = all_centros
    else:
        selected_centros = st.sidebar.multiselect("Selecciona Centro(s):", options=all_centros)

    if st.sidebar.checkbox("Seleccionar TODOS los años", value=False):
    ...
    filtered_contracts = contracts_df[
        ...
        (contracts_df["centro"].isin(selected_centros))
    ]
...
##############################################################################
    ...
    if not selected_years_consumo:
        ...
        # Agregar consumo total por centro y mes
...
##############################################################################
    ...
    st.markdown("<h2 style='font-size:24px;'>Selecciona un Centro y luego un Contrato para ver sus importes y datos generales.</h2>", unsafe_allow_html=True)

    all_centers = sorted(contracts_df["centro"].unique().tolist())
    selected_center = st.sidebar.selectbox("Selecciona un Centro:", options=["-- Ninguno --"] + all_centers)
    
    if selected_center != "-- Ninguno --":
        df_center = contracts_df[contracts_df["centro"] == selected_center]
        if df_center.empty:
        ...
        st.info("Selecciona un Centro para ver los contratos disponibles.")